...
const StockSpotlight = ({ spotlight, simple, callHeadline, callFrozen = false }) => {
  ...
  // The lesson posture belongs to the published frozen call, not intraday drift.
  const lock = callFrozen && spotlightLocked(callHeadline);
  const body = (

    <div role={simple && lock ? undefined : "region"} aria-label={simple && lock ? undefined : "Stock Spotlight"} className={simple && lock ? "stock-spotlight-content" : "stock-spotlight"}
      style={simple && lock ? { padding: "8px 0" } : { padding: "16px 20px", marginTop:12, background: T.bg, borderTop:`2px solid ${T.amber}66`, borderBottom: `1px solid ${T.border}` }}>
