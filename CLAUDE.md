...
Verification: browser-required gates passed 2,758 smoke checks plus 11 lock pins, 353
admin-browser and 861 public-browser checks; production audit zero vulnerabilities.
Production validation caught and corrected the earlier hero's publication-contract bug:
md-call-v1 uses published:true, not status:PUBLISHED. Both hero action and Spotlight now
share the correct predicate; fixtures are generated by the real publisher, not hand-written.
