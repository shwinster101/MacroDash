...
ok("dd: the payload cap states that the BOOK cap binds first (38 x 45KB > the 600KB MAX_BODY)",
  /binding constraint is the BOOK/.test(adminSrc) && /1\.7MB/.test(adminSrc));
...
ok("e2e: the positions store's smaller cap is documented, not left to look accidental",
  (() => { const src = readFileSync(new URL("../functions/api/positions.js", import.meta.url), "utf8");
    return /Deliberately 64KB, NOT the book's 600KB/.test(src); })());
ok("e2e: the book cap and its client pre-flight mirror still agree",
  /const MAX_BODY = 600 \* 1024;/.test(ttSrc) && /const MAX_BODY=600\*1024;/.test(adminSrc));
